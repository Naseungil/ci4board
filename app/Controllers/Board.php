...
class Board extends BaseController
{
  public function list(): string
  {
    ...
    $result = $db->query($sql);
    $data['list'] = $result -> getResult(); //조회결과를 변수명 data에 할당
    */
    ...
    //$data['pager_links'] = $pager_links 아래와 같은 코드
    $data = [
      'list' => $query,
      'total' => $total,
      'page' => $page,
  ...
  public function view($bid=null): string
  {
    ...
    $result = $db->query($sql);
    $data['view'] = $result -> getRow(); //조회결과를 변수명 data에 할당
    */
    $boardModel = new BoardModel();
  ...
  public function write()
  {
    if(!isset($_SESSION['userid'])){
      return redirect()->to('/login')->with('alert','로그인해주세요'); //with함수는 세션을 생성해줌
    }
